...
export function isExpressBackendEnabled(authData = null) {
  ...
  // 2. CANARY ALLOWLIST FOR SOLO DEV / TEST ACCOUNTS (Bypasses percentage sampling)
  ...
  // 3. GLOBAL FEATURE FLAG (Env or Remote Config)
  if (process.env.USE_EXPRESS_BACKEND === "true" || cachedRemoteConfig?.enabled === true) {
    // If explicitly disabled in env, prioritize env unless remote override is on
    if (process.env.USE_EXPRESS_BACKEND === "false" && !cachedRemoteConfig?.enabled) {
      return false;
    }
    return true;
  }

  // 4. CANARY PERCENTAGE SAMPLING
